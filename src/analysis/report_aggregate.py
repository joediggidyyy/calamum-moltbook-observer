...
def _publish_candidate(candidate: Mapping[str, Any]) -> None:
    ...
    report_markdown = source_report_paths['markdown'].read_text(encoding='utf-8')
    (publication_dir / 'report.md').write_text(report_markdown.replace('../figures/', 'figures/'), encoding='utf-8')
    shutil.copy2(source_report_paths['json'], publication_dir / 'report.json')
    shutil.copy2(source_report_paths['manifest'], publication_dir / 'manifest.json')
...
def _published_run_summary(candidate: Mapping[str, Any], project_root: Path) -> Dict[str, Any]:
    manifest_payload = dict(candidate.get('manifest_payload', {}) or {})
    publication_dir = Path(candidate['publication_dir'])
    source_report_paths = dict(candidate.get('source_report_paths', {}) or {})
    ...
    return {
        ...
        'decision': str(manifest_payload.get('decision', '')),
        'summary': str(manifest_payload.get('summary', '')),
        'source_run_root': str(manifest_payload.get('run_root', '')),
        'source_manifest_path': normalize_repo_or_absolute_path(source_report_paths.get('manifest'), project_root),
        'source_report_paths': {
