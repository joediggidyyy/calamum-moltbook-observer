...
def _classify(path: Path, operations_root: Path) -> Tuple[str, str]:
    ...
    first_bucket = rel.parts[0] if rel.parts else ''
    ...
    if rel.parent == Path('.'):
        ...
        if name.startswith('CALAMUM_REPORT_PATH_MIGRATION_INDEX_'):
            return 'KEEP', 'canonical migration index artifact'

    if name.startswith('CALAMUM_REPORT_PATH_MIGRATION_INDEX_'):
        return 'KEEP_OR_BUCKET', 'historical migration index artifact'
    if name.startswith('OBSERVERCTL_') and (
        '_VALIDATION_' in name or '_RUN_' in name or '_PACKET_' in name
    ):
    ...
    if first_bucket == 'compat_packets' and name.startswith('OBSERVERCTL_'):
        return 'KEEP_OR_BUCKET', 'observer compatibility evidence packet'
    if first_bucket == 'audits' and any(
        token in name for token in ('_ASSESSMENT_', '_AUDIT_', '_ANALYSIS_', '_INVESTIGATION_', '_SNAPSHOT_')
    ):
        return 'KEEP_OR_BUCKET', 'audit-bucket operational artifact'
    if first_bucket == 'standards' and any(
        token in name for token in ('_STANDARD_', '_CHECKLIST_', '_HANDOFF_')
    ):
        return 'KEEP_OR_BUCKET', 'standards-bucket operational artifact'
    if '_PLAN_' in name:
...
def test_operations_docs_have_no_review_replace_items() -> None:
    root = _project_root()
    operations_root = root / 'local_untracked' / 'reports' / 'operations'
    assert operations_root.exists(), 'local_untracked/reports/operations/ must exist'
...
def test_operations_docs_use_approved_bucket_subfolders() -> None:
    ...
    for p in operations_root.rglob('*'):
        ...
        if rel.parent == Path('.'):
            if not (
                p.name.startswith('JOB_REPORT_')
                or p.name.startswith('INCIDENT_REPORT_')
                or p.name.startswith('CALAMUM_REPORT_PATH_MIGRATION_INDEX_')
            ):
