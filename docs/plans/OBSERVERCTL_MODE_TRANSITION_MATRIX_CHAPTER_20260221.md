...
|---|---|---|---|
| C01 | `observer_service_heartbeat_fresh` | observer service/runtime heartbeat within freshness window (independent of collection activity) | critical |
...
| C23 | `collection_state_semantics_valid` | collection state (`idle|warmup|collecting|stopped|error`) is present and does not contradict runtime/service health | major |
| C24 | `resource_stream_retention_ready` | always-on resource stream is writable and librarian rotation/compression pipeline is healthy | critical |
| C25 | `resource_baseline_window_ready` | baseline rapid-sampling window and baseline-derived metrics are complete for live-readiness gate | critical |
...
- `critical_check_failed:collection_state_incoherent`
...
## 14.2 Resource telemetry retention and baseline contract

Resource telemetry is a first-class stream and is retained continuously (not only during active collection).

Required stream classes:
- `resource_normal`: fixed-interval sampling for long-run baseline continuity.
- `resource_baseline`: rapid-sampling bursts for rate-of-change and baseline-envelope derivation.
- `active_record`: live/honeypot collection stream (existing interaction metadata lane).

Librarian lifecycle obligations (all stream classes):
- rotate by policy window/size,
- compress archived segments,
- preserve per-segment stream metadata,
- maintain index continuity for replay/audit.

Run-time metadata minimums for resource streams:
- `stream_type` (`resource_normal|resource_baseline|active_record`)
- `sampling_profile_id`
- `mode_at_capture`
- `source_axis`
- `baseline_window_id` (required for rapid baseline segments)

Gate semantics:
- Stage 5 readiness gate validates **service heartbeat freshness** and baseline/resource contract completeness.
- Stage 5 readiness gate does **not** require collection state `collecting`; `idle|warmup|stopped` is valid pre-activation context.
- Live activation gate (separate step) requires transition to `collecting` within policy timeout.
...
- [ ] Resource telemetry (`resource_normal` + `resource_baseline`) is retained continuously with librarian rotation/compression.
- [ ] Service heartbeat semantics are decoupled from collection activity semantics in gate decisions.
