...
def _extract_feature_row(rec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    row: Dict[str, Any] = {
        ...
        'content_length_words': _safe_int(rec.get('content_length_words'), 0),
        'has_code_block': _safe_bool(rec.get('has_code_block')),
        'code_block_count': _safe_int(rec.get('code_block_count'), 0),
        'has_link': _safe_bool(rec.get('has_link')),
        'link_count': _safe_int(rec.get('link_count'), 0),
        'tags_count': _safe_int(rec.get('tags_count'), 0),
        'mentions_count': _safe_int(rec.get('mentions_count'), 0),
        'line_count': _safe_int(rec.get('line_count'), 0),
        'question_count': _safe_int(rec.get('question_count'), 0),
        'exclamation_count': _safe_int(rec.get('exclamation_count'), 0),
        'contains_ignore_previous': _safe_bool(rec.get('contains_ignore_previous')),
        'contains_system_prompt_reference': _safe_bool(rec.get('contains_system_prompt_reference')),
        'contains_developer_message_reference': _safe_bool(rec.get('contains_developer_message_reference')),
        'contains_env_var_reference': _safe_bool(rec.get('contains_env_var_reference')),
        'prompt_injection_score': _safe_int(rec.get('prompt_injection_score'), 0),
        'matched_pattern_count': _safe_int(rec.get('matched_pattern_count'), 0),
